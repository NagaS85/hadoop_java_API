...
public class DataIngestionCallable implements Callable<String> {
	...
	final static Logger logger = LoggerFactory.getLogger(DataIngestionCallable.class);
	 ...
	 */
	public DataIngestionCallable(String inPath, String fileName,
			String outPath, CyclicBarrier barrier, 
			//ReentrantLock l,
			String fileType) {
		this.inPath = inPath;
	...
	public String call() throws Exception {
		try {
			...
			logger.debug(Thread.currentThread().getName() + " is waiting on barrier");
			barrier.await();
			logger.debug(Thread.currentThread().getName() + " has crossed the barrier");
		} catch (IOException e) {
			e.printStackTrace();
		}
